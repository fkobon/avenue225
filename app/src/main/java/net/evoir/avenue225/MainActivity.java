	...
	// change drawer width
	 mDrawerList.post(new Runnable() {
	        ...
	        public void run() {
	            ...
	            float width = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 300, resources.getDisplayMetrics());
  ...
  private ArrayList<Category> getCategories() {
	// TODO Auto-generated method stub
		try {
            ...
            listCategories = Model.getHelper(mContext).getDao(Category.class).queryForAll();
			categories.clear();
            ...
            if(listCategories.size()>0) {
                for(int i = 0; i < listCategories.size(); i++){
                    ...
                    Category categoryItem = new Category(R.drawable.logo,listCategories.get(i).getTitle(),
                            -1,listCategories.get(i).getNumber());
                    categoryItem.setSlug(listCategories.get(i).getSlug());
...
private boolean isDataBaseNotEmtpy() {
	boolean answer=true;
	List<Post> dbPostlist = new ArrayList<Post>();
    try {
		//QueryBuilder<Post, String> queryBuilder =
		//dao.queryBuilder();
		//queryBuilder.limit(50);
		//dbPostlist = queryBuilder.query();
		dbPostlist = Model.getHelper(mContext).getDao(Post.class).queryForAll();
...
public void selectItem(int position)
  {
	...
	if (position == categories.size()-1){
		...
		startActivity(i);
	      ...
	      args.putInt("categoryNumber", categories.get(position).getNumber());
